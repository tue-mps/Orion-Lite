...
# Update this path to point to your trained checkpoint from distill/runs
mimic_model_path = '/mnt/adas7tb/jgu/Orion/distill/runs/20251214_124846/checkpoints/best.pt'  # Path to trained OrionMimicModel checkpoint
mimic_input_dim = 4096
mimic_hidden_dim = 1024
mimic_output_dim = 4096
mimic_num_layers = 6
mimic_num_heads = 16
mimic_dropout = 0.1
mimic_with_bound_loss = True
mimic_use_col_loss = True
...
model = dict(
    type='OrionWithoutLLM',
    mimic_model_path=mimic_model_path,  # Path to trained OrionMimicModel checkpoint
    save_path='./results_planning_only/',  #save path for planning results.
    use_grid_mask=True,
    fp16_infer=fp16_infer,
    fp16_eval=fp16_infer,
    use_diff_decoder = False,
    use_col_loss=use_col_loss,
    # OrionMimicModel config (replaces both LLM and VAE)
    input_dim=mimic_input_dim,
    hidden_dim=mimic_hidden_dim,
    output_dim=mimic_output_dim,
    num_layers=mimic_num_layers,
    num_heads=mimic_num_heads,
    dropout=mimic_dropout,
    with_bound_loss=mimic_with_bound_loss,
    img_backbone=dict(
        type='EVAViT',
        img_size=640, 
        patch_size=16,
    ...
    dict(
        ...
        transforms=[
            ...
            dict(
                ...
                # Removed input_ids, vlm_labels - no longer needed without LLM
                keys=['gt_bboxes_3d', 'gt_labels_3d', 'img', 'ego_his_trajs','gt_attr_labels', 'ego_fut_trajs', 'ego_fut_masks','ego_fut_cmd', 'ego_lcf_feat','can_bus','fut_valid_flag']+collect_keys,
            )]
    )
]
