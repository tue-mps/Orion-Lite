...
def parse_args():
    ...
    parser.add_argument(
        '--compile-backbone',
        action='store_true',
        help='Compile the vision backbone with torch.compile')
    parser.add_argument(
        '--compile-mode',
        default='default',
...
def main():
    ...
    if args.compile_backbone:
        ...
        elif not hasattr(model, 'img_backbone'):
            print('compile-backbone requested, but model has no img_backbone.')
        else:
            print(f'Compiling img_backbone with torch.compile(mode={args.compile_mode})...')
            model = model.cuda()
            try:
                model.img_backbone = torch.compile(
                    model.img_backbone, mode=args.compile_mode
                )
            except Exception as exc:
                print(f'torch.compile failed: {exc}')
                print('Continuing with eager img_backbone.')
