#!/bin/bash
# Evaluation script for Orion-Lite student model trained WITH feature-mimic loss.
# Student decoder: 6 layers, with mimic (L1) loss.
# Corresponds to: distill/results/orion_student_with_mimic_loss/
#
# Usage:
#   bash eval/scripts/run_orion_student_with_mimic_loss_multi_strict.sh [preflight|full|postprocess|all]
#
...
ALGO="orion_student_6l_with_mimic_strict"
CONFIG_PATH="${ORION_LITE_ROOT}/eval/configs/orion_student_6l_with_mimic_b2d_agent_eval.py"
CKPT_PATH="${CKPT_PATH:-${ORION_LITE_ROOT}/eval/merged_ckpts/orion_student_with_mimic_loss.pth}"
BASE_PORT="${USER_BASE_PORT:-31400}"
BASE_TM_PORT="${USER_BASE_TM_PORT:-51400}"
...
run_postprocess() {
  ...
  python tools/ability_benchmark.py -f "${BASE_ROUTES}.xml" -r "${merged_json}" -p "${ability_port}" | tee "${RESULT_DIR}/ability.log"
  python tools/efficiency_smoothness_benchmark.py -f "${merged_json}" -m "${SAVE_PATH}" | tee "${RESULT_DIR}/efficiency_smoothness.log"
