...
ALGO="orion_student_exp9_stage2_strict"
CONFIG_PATH="${ORION_LITE_ROOT}/eval/configs/orion_student_exp9_stage2_b2d_agent_eval.py"
CKPT_PATH="${CKPT_PATH:-}"  # Required: set CKPT_PATH to exp9 stage2 checkpoint
BASE_PORT="${USER_BASE_PORT:-30900}"
BASE_TM_PORT="${USER_BASE_TM_PORT:-50900}"
...
TEAM_CONFIG="${CONFIG_PATH}+${CKPT_PATH}+exp9_stage2_strict_multi"
RESULT_DIR="${ALGO}_b2d_${PLANNER_TYPE}"
SAVE_PATH="${SAVE_PATH:-./eval_bench2drive220_${ALGO}_${PLANNER_TYPE}}"

PRECHECK_LOG="preflight_exp9_${PLANNER_TYPE}.log"
...
run_preflight() {
  echo "[exp9] Preflight on Dev10 routes..."
  ...
  echo "[exp9] Preflight passed."
}

run_full() {
  echo "[exp9] Full B2D220 run (${TASK_NUM} tasks)..."
  mkdir -p "${RESULT_DIR}"
  ...
  echo "[exp9] Full run finished."
}

run_postprocess() {
  echo "[exp9] Post-processing metrics..."
...
PY
  echo "[exp9] Wrote summary: ${summary_json}"
}
