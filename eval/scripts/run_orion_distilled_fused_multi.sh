#!/bin/bash
# Generic closed-loop Bench2Drive evaluation for fused OrionDistilled checkpoints.
#
# Required env:
#   CONFIG_PATH : distilled agent config (fused variant)
#   CKPT_PATH   : single fused checkpoint produced by export_distill_fused_ckpt.py
#   ALGO        : experiment tag used for output folders
...
CKPT_PATH="${CKPT_PATH:?CKPT_PATH must be set}"
ALGO="${ALGO:?ALGO must be set}"
BASE_PORT="${USER_BASE_PORT:-32400}"
BASE_TM_PORT="${USER_BASE_TM_PORT:-52400}"
