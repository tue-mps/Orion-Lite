#!/bin/bash
# Evaluation script for Orion-Lite student model trained WITHOUT feature-mimic loss.
# Student decoder: 6 layers, no mimic loss.
# Corresponds to: distill/results/orion_student_no_mimic_loss/
#
# Usage:
#   bash eval/scripts/run_orion_student_no_mimic_loss_multi_strict.sh [preflight|full|postprocess|all]
#
...
ALGO="orion_student_6l_no_mimic_strict"
CONFIG_PATH="${ORION_LITE_ROOT}/eval/configs/orion_student_6l_no_mimic_b2d_agent_eval.py"
CKPT_PATH="${CKPT_PATH:-${ORION_LITE_ROOT}/eval/merged_ckpts/orion_student_no_mimic_loss.pth}"
BASE_PORT="${USER_BASE_PORT:-31200}"
BASE_TM_PORT="${USER_BASE_TM_PORT:-51200}"
