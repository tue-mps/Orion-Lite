...
class OrionDistilled(MVXTwoStageDetector):
    def __init__(self,
                 ...
                 use_student_decoder=False,
                 compile_img_backbone=False,
                 compile_img_backbone_mode='reduce-overhead',
                 compile_img_backbone_backend=None,
                 compile_suppress_errors=True,
                 ):
        super(OrionDistilled, self).__init__(pts_voxel_layer, pts_voxel_encoder,
        ...
        self.compile_img_backbone = compile_img_backbone
        if self.compile_img_backbone:
            if not hasattr(torch, "compile"):
                if os.environ.get("RANK", "0") == "0":
                    print("[torch.compile] torch.compile() not available (requires PyTorch 2.0+); img_backbone will run in eager mode.", flush=True)
            else:
                if compile_suppress_errors:
                    try:
                        from torch import _dynamo as torch_dynamo  # type: ignore
                        torch_dynamo.config.suppress_errors = True
                    except Exception:
                        pass
                compile_kwargs = {"mode": compile_img_backbone_mode}
                if compile_img_backbone_backend is not None:
                    compile_kwargs["backend"] = compile_img_backbone_backend
                self.img_backbone = torch.compile(self.img_backbone, **compile_kwargs)
                if os.environ.get("RANK", "0") == "0":
                    print(
                        f"[torch.compile] img_backbone compiled: mode={compile_img_backbone_mode!r} backend={compile_img_backbone_backend!r} "
                        f"type={type(self.img_backbone)} has__orig_mod={hasattr(self.img_backbone, '_orig_mod')}",
                        flush=True,
                    )
        else:
            if os.environ.get("RANK", "0") == "0":
                print("[torch.compile] img_backbone compile disabled (compile_img_backbone=False).", flush=True)

        self.use_student_decoder = use_student_decoder
        self.student_model = None
        self._student_model_device_initialized = False  # Track if student model device has been set
        if self.use_student_decoder or student_model_path is not None:
            if student_model_path is not None:
                print(f"Loading Student Model from {student_model_path}...")
            # Ensure these dimensions match your trained student
            self.student_model = OrionStudent(
                input_dim=4096,        # Match 'input_dim'
                output_dim=4096,       # Match 'output_dim'
                hidden_dim=1024,       # Match 'hidden_dim' (check your training config!)
                num_heads=16,          # Match 'num_heads' (not n_head)
                num_layers=6           # Match 'num_layers' (not n_layer)
            )
            if student_model_path is not None:
                checkpoint = torch.load(student_model_path, map_location='cpu')
                self.student_model.load_state_dict(checkpoint['model_state_dict'])
            if not self.use_student_decoder:
                self.student_model.eval() # Freeze student

    def init_weights(self):
        try:
            super().init_weights()
        except AttributeError as e:
            if '_params_init_info' in str(e):
                for m in self.modules():
                    if hasattr(m, '_params_init_info'):
                        try:
                            delattr(m, '_params_init_info')
    ...
    def forward_pts_train(self,
        ...
        if self.with_lm_head:
            if self.use_gen_token:
                ...
                losses.update(vlm_loss=vlm_loss[0])
        elif self.use_student_decoder:
            vision_embeded = vision_embeded_obj
            if self.with_map_head:
                vision_embeded = torch.cat([vision_embeded_obj, vision_embeded_map], dim=1)
            target_device = vision_embeded.device
            if not self._student_model_device_initialized:
                self.student_model = self.student_model.to(target_device)
                if torch.cuda.is_available():
                    torch.cuda.synchronize()
                self._student_model_device_initialized = True
            else:
                student_device = next(self.student_model.parameters()).device
                if student_device != target_device:
                    self.student_model = self.student_model.to(target_device)
                    if torch.cuda.is_available():
                        torch.cuda.synchronize()

            current_states = self.student_model(vision_embeded)
            ego_feature = current_states.squeeze(1)
            current_states = ego_feature.unsqueeze(1)
    ...
    def simple_test_pts(self, img_metas, **data):
        ...
        if self.student_model is not None:
            with CodeTimer("5. Student Model"):
                vision_embeded = torch.cat([vision_embeded_obj, vision_embeded_map], dim=1) # (1, 513, 4096)
                
                # Optimization: Run student model ONCE as it only depends on vision, not text inputs
                # Ensure student_model is on the correct device (only move once to prevent memory issues/segfaults)
                target_device = vision_embeded.device
                if not self._student_model_device_initialized:
                    # First time: move to target device
                    student_device = next(self.student_model.parameters()).device
                    if student_device != target_device:
                        self.student_model = self.student_model.to(target_device)
                        if torch.cuda.is_available():
                    ...
                    # After initialization, verify device matches (should be on correct device already)
                    student_device = next(self.student_model.parameters()).device
                    if student_device != target_device:
                        # Device mismatch - this shouldn't happen, but handle it safely
                        print(f"Warning: Student model device mismatch ({student_device} vs {target_device}). Moving model.")
                        self.student_model = self.student_model.to(target_device)
                        if torch.cuda.is_available():
                            torch.cuda.synchronize()
                with torch.no_grad():
                ...
                # Squeeze to get [B, 4096] if needed, or keep [B, 1, 4096] depending on downstream
                ego_feature = current_states.squeeze(1).to(torch.float32)
                current_states = ego_feature.unsqueeze(1)
                # We don't generate text with the student, so we can leave generated_text empty or dummy
                generated_text = [{"Q": "Student", "A": "Trajectory generated by Student Model"}]
            
            with CodeTimer("6. Trajectory Decoder"):
                ...
                if not self.use_diff_decoder and not self.use_mlp_decoder: # VAE-based generate 
                    ...
                    if self.PROBABILISTIC:
                        ...
                        # Fallback: use current_states as sample if PROBABILISTIC is False
