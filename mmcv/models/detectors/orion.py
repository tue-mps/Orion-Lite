        ...
        if self.with_lm_head:
            ...
            for i, input_ids in enumerate(data['input_ids'][0]):
                ...
                if self.use_gen_token and special_token_inputs: # must be the final round conversation
                    ...
                    llm_total_ms += self._timer_end_ms(llm_start)
                    ...
                    # import numpy as np
                    # import os
                    # save_dir = '/mnt/adas7tb/jgu/Orion/distill/distill_data/val'
                    # os.makedirs(save_dir, exist_ok=True)

                    # # Use timestamp/scene_token for unique filename
                    # # Note: img_metas is a list of dicts here
                    ...
                    # filename = f"{save_dir}/data_{safe_scene_token}_{unique_id}.npy"
                    ...
                    #         # Process lane_preds same as training code
                    #         for p in range(self.map_head.n_control):
                    #             lane_preds_gt[..., 3 * p].clamp_(min=self.map_head.pc_range[0], max=self.map_head.pc_range[3])
                    #             lane_preds_gt[..., 3 * p + 1].clamp_(min=self.map_head.pc_range[1], max=self.map_head.pc_range[4])
                    #         lane_preds_gt = lane_preds_gt.reshape(lane_preds_gt.shape[0], lane_preds_gt.shape[1], -1, 3)[..., :2]

                    # # Extract agent_outs for collision loss (same tensors as training)
                    # agent_outs_gt = None
                    ...
                    # agent_fut_preds = all_traj_preds[-1].view(batch, num_agent, self.pts_bbox_head.fut_mode, self.pts_bbox_head.fut_ts, 2)
                    # agent_fut_cls_preds = all_traj_cls_scores[-1].view(batch, num_agent, self.pts_bbox_head.fut_mode)
                    # agent_outs_gt = {
                    ...
                    # # Explicitly cast to float16 to save space
                    # # The instruction (token IDs) should be int, not float
                    # save_data = {
                    #     'visual_queries': vision_embeded.cpu().numpy().astype(np.float16),
                    #     'planning_token': current_states.cpu().numpy().astype(np.float16)
                    # }
                    # # Add ground truth trajectory data if available
                    ...
                    if not self.use_diff_decoder and not self.use_mlp_decoder: # VAE-based generate 
                        ...
                        ego_fut_preds = torch.stack(ego_fut_trajs_list, dim=2)
                        vae_total_ms += self._timer_end_ms(vae_start)
                    elif self.use_diff_decoder:
                        diff_start = self._timer_start()
                        step_num = 2
                        ...
                        diff_total_ms += self._timer_end_ms(diff_start)
                    elif self.use_mlp_decoder:
                        mlp_start = self._timer_start()
                        waypoint = self.waypoint_decoder(current_states)
                        waypoint = waypoint.reshape(-1,2)
                        mlp_total_ms += self._timer_end_ms(mlp_start)
                else:
                    history_input_output_id.append(input_ids)
                    context_input_ids = torch.cat(history_input_output_id,dim=-1)
                    llm_start = self._timer_start()
            ...
            timing_ms['llm_ms'] = llm_total_ms
            if vae_total_ms > 0.0:
                timing_ms['vae_ms'] = vae_total_ms
            if diff_total_ms > 0.0:
                timing_ms['diff_decoder_ms'] = diff_total_ms
            if mlp_total_ms > 0.0:
                timing_ms['mlp_decoder_ms'] = mlp_total_ms
