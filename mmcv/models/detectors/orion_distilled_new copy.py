...
class OrionDistilledNew(MVXTwoStageDetector):
    def __init__(self,
                 ...
                 # Added args for OrionMimicModel if needed, but defaults in OrionMimicModel are usually fine or we rely on them
                 ):
        ...
        if student_model_path is not None:
            ...
            # Initialize OrionMimicModel
            # Note: We assume standard dims from OrionStudent, or we should use args if provided.
            # Here using defaults/inferred from context similar to OrionStudent usage
            self.student_model = OrionMimicModel(
                input_dim=4096,
                hidden_dim=1024,
                output_dim=4096,
                num_layers=6,
                ...
                # Pass loss configs to match Orion
                loss_plan_reg=loss_plan_reg,
                loss_plan_bound=loss_plan_bound,
                loss_plan_col=loss_plan_col,
                loss_vae_gen=loss_vae_gen,
                # loss_feature_mimic is default
            )
            
            # Load weights manually to avoid prefix issues if loaded via mmcv
            ...
            # The checkpoint keys are likely "input_proj.weight" etc. (no prefix)
            # OrionMimicModel expects these exact keys.
            self.student_model.load_state_dict(state_dict, strict=True)
            print("Student Model loaded with strict=True.")
            
            self.student_model.eval() 
        else:
            print("Warning: No student_model_path provided for OrionDistilledNew!")
